...
def validate_old(
    ...
    for idx, val_data in enumerate(val_dataloader):
        ...
        losses_topk = compute_deterministic_nll(
            val_data, model, strategy="topk-confidence"
        )
        losses_topk_accum += losses_topk.sum().item()

        losses_block_topk = compute_deterministic_nll(
            val_data, model, strategy="block-topk-confidence"
        )
        losses_block_topk_accum += losses_block_topk.sum().item()

        losses_topk_margin = compute_deterministic_nll(
            val_data, model, strategy="topk-margin"
        )
        losses_topk_margin_accum += losses_topk_margin.sum().item()

        losses_confidence_thresh = compute_deterministic_nll(
            val_data, model, strategy="confidence threshold"
        )
        losses_confidence_thresh_accum += losses_confidence_thresh.sum().item()

        losses_autoregressive = compute_deterministic_nll(
            val_data, model, strategy="autoregressive"
        )
        losses_autoregressive_accum += losses_autoregressive.sum().item()

        # print the accumulated losses divided by the counter
        print(
            f"Val iter {idx}:\tNLL Bound {losses_mc_accum.item() / counter.item():.4f},\tNLL Top-K {losses_topk_accum.item() / counter.item():.4f},\tNLL Confidence Threshold {losses_confidence_thresh_accum.item() / counter.item():.4f},\tNLL Top-K Margin {losses_topk_margin_accum.item() / counter.item():.4f},\tNLL Block Top-K {losses_block_topk_accum.item() / counter.item():.4f},\tNLL Autoregressive {losses_autoregressive_accum.item() / counter.item():.4f}\tCounted Samples {counter.item()}"
        )
    ...
    total_counter = fabric.all_reduce(
        torch.tensor(counter, device=fabric.device), reduce_op="sum"
    ).item()

    losses_mc = (
        fabric.all_reduce(
            torch.tensor(losses_mc_accum, device=fabric.device), reduce_op="sum"
        )
        / total_counter
    )
    ...
    losses_topk = (
        fabric.all_reduce(
            torch.tensor(losses_topk_accum, device=fabric.device), reduce_op="sum"
        )
        / total_counter
    )
    ...
    losses_block_topk = (
        fabric.all_reduce(
            torch.tensor(losses_block_topk_accum, device=fabric.device), reduce_op="sum"
        )
        / total_counter
    )

    losses_confidence_thresh = (
        fabric.all_reduce(
            torch.tensor(losses_confidence_thresh_accum, device=fabric.device),
            reduce_op="sum",
        )
        / total_counter
    )
    ...
    losses_topk_margin = (
        fabric.all_reduce(
            torch.tensor(losses_topk_margin_accum, device=fabric.device),
            reduce_op="sum",
        )
        / total_counter
    )
    ...
    losses_autoregressive = (
        fabric.all_reduce(
            torch.tensor(losses_autoregressive_accum, device=fabric.device),
            reduce_op="sum",
        )
        / total_counter
    )
    ...
    if fabric.global_rank == 0:
        ...
        print(
            f"Final Validation NLL Confidence Threshold: {losses_confidence_thresh.item():.4f}"
        )
        ...
        print(
            f"Final Validation NLL Autoregressive: {losses_autoregressive.item():.4f}"
        )
...
def _validate_mc_old(
    ...
    for i in range(mc_num):
        ...
        print(
            f"Loss shape: {loss.shape}, Input shape: {input_ids.shape}, Mask shape: {mask_indices.shape}, P_mask shape: {p_mask.shape}"
        )
        loss = loss.sum() / (
            input_ids.shape[0] * input_ids.shape[1]
        )  # normalized by total number of tokens
...
def greedy_and_uniform_decoding(
    ...
    for k in range(seq_len):
        ...
        nll_k_greedy = -masked_log_probs[
            torch.arange(batch_size), greedy_position
        ]  # [batch_size]
        z_k[torch.arange(batch_size), greedy_position] = x[
            torch.arange(batch_size), greedy_position
        ]  # [batch_size, seq_len]
        ...
        positions_to_mask = random_permutations[
            :, :num_masked
        ]  # [batch_size * num_monte_carlo, num_masked]

        x_all = x.repeat_interleave(
            num_monte_carlo, dim=0
        )  # [batch_size * num_monte_carlo, seq_len]
        ...
        logits_all = model(
            z_k_all
        )  # [batch_size * num_monte_carlo, seq_len, vocab_size]
        token_log_probs_all = F.log_softmax(
            logits_all, dim=-1
        )  # [batch_size * num_monte_carlo, seq_len, vocab_size]
        ...
        nelbo_all_samples = (
            -masked_log_probs_all.sum(dim=-1) / num_masked
        )  # [batch_size * num_monte_carlo]

        nelbo_samples = nelbo_all_samples.view(
            batch_size, num_monte_carlo
        )  # [batch_size, num_monte_caro]
        nelbo_k_uniform = nelbo_samples.mean(dim=1)  # [batch_size]
        nelbo_uniform_list.append(nelbo_k_uniform)

        print(
            f"Num unmasked tokens {k}:\tNLL Greedy: {nll_k_greedy}\tNELBO Uniform: {nelbo_k_uniform}"
        )
    ...
    print(
        f"NLL Greedy: {nll_greedy_per_token}\tNELBO Uniform: {nelbo_uniform_per_token}"
    )
...
def test_nelbo_equivalence(model, x, t_frac=0.9, eps=1e-3):
    ...
    # ===== METHOD 1 STYLE =====
    token_losses_1 = (
        F.cross_entropy(logits[mask_indices], x[mask_indices], reduction="none")
        / p_mask[mask_indices]
    )
    ...
    batch_ids_masked = (
        ...
        .unsqueeze(1)
        .expand_as(mask_indices)[mask_indices]
    )
    ...
    log_probs_masked = torch.where(
        mask_indices, log_probs_true, torch.zeros_like(log_probs_true)
    )
